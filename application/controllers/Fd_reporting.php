...
class Fd_reporting extends Root_Controller
{
    ...
    private function system_list()
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1))
        {
            ...
            $data = array();
            $data['system_preference_items'] = System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title'] = "Report Pending List";
            $ajax['status'] = true;
    ...
    private function system_reporting($id)
    {
        if (isset($this->permissions['action2']) && ($this->permissions['action2'] == 1))
        {
            ...
            $this->db->from($this->config->item('table_ems_fd_budget') . ' fd_budget');
            $this->db->select('fd_budget.date_proposal, fd_budget.status_budget_forward, fd_budget.status_recommendation, fd_budget.status_approve, fd_budget.status_payment_approve');
            ...
            {
                System_helper::invalid_try(__FUNCTION__, $item_id, 'Trying to Report Field Day of other Location');
                $ajax['status'] = false;
                $ajax['system_message'] = 'Trying to Report Field Day of other Location';
                $this->json_return($ajax);
            }
