...
class Fd_approve extends Root_Controller
{
    ...
    private function system_edit($id)
    {
        if (isset($this->permissions['action2']) && ($this->permissions['action2'] == 1))
        {
            ...
            $ajax = Fd_budget_helper::fd_budget_status_check($result, array(FD_RECOMMENDATION_FORWARDED, FD_BUDGET_NOT_APPROVED));
            if (!$ajax['status'])
            {
                $this->json_return($ajax);
            }
...
/*
        ...
        private function system_forward($id)
        {
            if (isset($this->permissions['action7']) && ($this->permissions['action7'] == 1))
            {
                ...
                $data['title'] = "Forward Field Day Budget Recommendation ( ID:" . $result['budget_id'] . " )";
                $ajax['status'] = true;
