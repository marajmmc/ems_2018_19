...
class Da_tmpo_setup_lead_farmer_dealer_variety extends Root_Controller
{
    ...
    private function system_save_variety()
    {
        ...
        else
        {
            ...
            $result=Query_helper::get_info($this->config->item('table_ems_da_tmpo_setup_area_varieties'),'*',array('variety_id='.$item['variety_id'], 'status !="'.$this->config->item('system_status_delete').'"'),1);
            if($result)
            {
                $ajax['status']=false;
