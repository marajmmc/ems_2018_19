...
class Da_tmpo_setup_area extends Root_Controller
{
    ...
    public function index($action="list",$id=0,$id1=0)
    {
        ...
        elseif($action=="area_setup")
        {
            $this->system_area_setup($id);
        }
        ...
        {
            $this->system_add_area($id);
        }
        elseif($action=='edit_area')
        {
            $this->system_edit_area($id,$id1);
        }
        elseif($action=="save")
        {
            $this->system_save();
    ...
    private function system_area_setup($id)
    {
        if(isset($this->permissions['action2']) && ($this->permissions['action2']==1))
        {
            ...
            if(!$data['item'])
            {
                System_helper::invalid_try('Area_setup',$item_id,'Id Non-Exists');
                $ajax['status']=false;
            ...
            $ajax['system_page_url']=site_url($this->controller_url.'/index/area_setup/'.$item_id);
            $this->json_return($ajax);
        }
        else
        {
    ...
    private function system_add_area($id)
    {
        if(isset($this->permissions['action2']) && ($this->permissions['action2']==1))
        {
            if($id>0)
            ...
            $ajax['system_page_url']=site_url($this->controller_url.'/index/add_area/'.$item_id);
            $this->json_return($ajax);
        }
        else
        {
    ...
    private function system_edit_area($outlet_id,$id)
    {
        if(isset($this->permissions['action2']) && ($this->permissions['action2']==1))
        {
            if($id>0)
            ...
            $ajax['system_page_url']=site_url($this->controller_url.'/index/edit_area/'.$outlet_id.'/'.$item_id);
            $this->json_return($ajax);
        }
        else
        {
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            if ($this->db->trans_status() === TRUE)
            {
                ...
                if($save_and_new==1)
                {
                    $this->system_add_area($item['outlet_id']);
                }
                else
                {
                    $this->system_area_setup($item['outlet_id']);
                }
            }
            else
            {
