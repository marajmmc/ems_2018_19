...
class Da_tmpo_setup_area extends Root_Controller
{
    ...
    public function index($action="list",$id=0,$id1=0)
    {
        ...
        elseif($action=='add_edit_schedule')
        {
            $this->system_add_edit_schedule($id);
        }
        elseif($action=="save_schedule")
        {
            $this->system_save_schedule();
        }
    ...
    private function system_get_items()
    {
        ...
        $this->db->join($this->config->item('table_ems_da_tmpo_setup_visit_schedules').' visit_schedules','visit_schedules.outlet_id = outlet_info.customer_id','LEFT');
        $this->db->select("IF(COUNT(visit_schedules.id)>0, 'YES', 'NO') status_schedule_setup");
    ...
    private function system_add_edit_schedule($id)
    {
        if($id>0)
        {
            $item_id=$id;
        ...
        {
            $ajax['status']=false;
            $ajax['system_message']='Growing area not setup. Before setup growing area and then make visit schedule.';
            $this->json_return($ajax);
        }
        ...
        $ajax['status']=true;
        $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view($this->controller_url."/add_edit_schedule",$data,true));
        if($this->message)
        {
            $ajax['system_message']=$this->message;
        }
        $ajax['system_page_url']=site_url($this->controller_url.'/index/add_edit_schedule/'.$item_id);
        $this->json_return($ajax);
    }
    private function system_save_schedule()
    {
        $id = $this->input->post("id");
        $item=$this->input->post('item');
