...
class Tm_ti_attendance extends Root_Controller
{
    private $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Tm_ti_attendance');
        ...
        $this->controller_url='tm_ti_attendance';
    }
    ...
    private function system_list()
    {
        ...
        {
            $data['title']="TI Attendance (Dealer And Farmer visit) Pending List";
            $ajax['status']=true;
...
//        exit;
    ...
    private function system_list_all()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            $data['title']="TI Attendance (Dealer And Farmer visit) All List";
            $ajax['status']=true;
    ...
    private function system_save()
    {
        ...
        $result=Query_helper::get_info($this->config->item('table_ems_tm_dealer_and_farmer_visit'),array('*'),array('id ='.$id,'status ="'.$this->config->item('system_status_active').'"'),1);
        if(!(isset($this->permissions['action2'])&&($this->permissions['action2']==1)))
        {
            $ajax['status']=false;
            $ajax['system_message']=$this->lang->line("YOU_DONT_HAVE_ACCESS");
